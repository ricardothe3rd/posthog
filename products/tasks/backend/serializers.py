...
class TaskRunCreateRequestSerializer(serializers.Serializer):
    ...
    PROVIDER_CHOICES = [provider.value for provider in LLMProvider]
    ...
    provider = serializers.ChoiceField(
        ...
        required=False,
        default=None,
        help_text="LLM provider backing the selected runtime adapter.",
    )
    ...
    def validate(self, attrs):
        runtime_fields = ("runtime_adapter", "provider", "model")
        ...
        expected_provider_by_adapter = {
            RuntimeAdapter.CLAUDE.value: LLMProvider.ANTHROPIC.value,
            RuntimeAdapter.CODEX.value: LLMProvider.OPENAI.value,
        }
        expected_provider = expected_provider_by_adapter.get(runtime_adapter)
        if (
            ...
            errors["provider"] = f"Provider '{provider}' is not valid for runtime_adapter '{runtime_adapter}'."
