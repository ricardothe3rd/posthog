...
class TestTaskAPI(BaseTaskAPITest):
    ...
    @parameterized.expand(
        [
            ("missing_provider", {"runtime_adapter": "codex", "model": "gpt-5.3-codex"}, "provider"),
            ("missing_model", {"runtime_adapter": "codex", "provider": "openai"}, "model"),
        ]
    )
    @patch("products.tasks.backend.api.execute_task_processing_workflow")
    ...
    @patch("products.tasks.backend.api.execute_task_processing_workflow")
    def test_run_endpoint_rejects_mismatched_runtime_provider(self, mock_workflow):
        task = self.create_task()
        ...
        assert response.json() == {
            "type": "validation_error",
            "code": "invalid_input",
            "detail": "Provider 'anthropic' is not valid for runtime_adapter 'codex'.",
            "attr": "provider",
        }
        mock_workflow.assert_not_called()
