...
def send_matview_failure_digest() -> None:
    ...
    for paused_sq in paused_queries:
        if str(paused_sq.id) in failed_query_ids_seen:
            continue
    ...
    if not teams_with_issues:
        logger.warning("No matview failures or paused schedules found")
        return
...
def send_team_matview_failure_digest(team_id: int, failed_query_ids: list[str], paused_query_ids: list[str]) -> None:
    ...
    for qid in failed_query_ids:
        sq = queries.get(qid)
        if sq:
            failed_job = latest_jobs.get(qid)
            error = (failed_job.error if failed_job else None) or sq.latest_error or "Unknown error"
            run_at = (failed_job.last_run_at if failed_job else None) or sq.last_run_at
            failed_views.append(
                {
                    "id": str(sq.id),
                    "name": sq.name,
    ...
    logger.info(
        "Sent materialized view failure digest email for team %d: %d failed, %d paused",
        team_id,
        len(failed_views),
        len(paused_views),
    )
