...
import { RecordingsQuery, LogMessage } from '~/queries/schema/schema-general'
...
export const playerInspectorLogic = kea<playerInspectorLogicType>([
        ...
        allItems: [
            ...
            (
                ...
                for (const log of logs || []) {
                    const logTimestamp = dayjs(log.timestamp)
                    const { timestamp, timeInRecording } = timeRelativeToStart(
                        { timestamp: logTimestamp.valueOf() },
                        start || dayjs()
                    )
                    ...
                    addItem({
                        ...
                        timeInRecording,
                        data: log,
                        search: `${log.level} ${log.body}`.toLowerCase(),
                        highlightColor,
                        key: `log-${log.uuid}`,
    ...
    listeners(({ values, actions }) => ({
        ...
        loadEventsSuccess: () => {
            ...
            // Load logs once session data is available (start/end are now populated)
            if (values.start && !values.logsLoading && values.logs.length === 0) {
                actions.loadLogs()
            }
        },
    })),
